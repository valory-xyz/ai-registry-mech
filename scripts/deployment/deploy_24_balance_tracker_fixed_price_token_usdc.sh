...
balanceTrackerFixedPriceTokenUSDCAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#balanceTrackerFixedPriceTokenUSDCAddress}
...
echo "$(jq '. += {"balanceTrackerFixedPriceTokenUSDCAddress":"'$balanceTrackerFixedPriceTokenUSDCAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$balanceTrackerFixedPriceTokenUSDCAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $balanceTrackerFixedPriceTokenUSDCAddress${reset}"